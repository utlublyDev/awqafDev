...
import { getEntities } from "./offers-categories.reducer";
...
export const OffersCategories = (
  ...
  const offersCategoriesList = useAppSelector(
  ...
  const handlePagination = (currentPage) =>
    ...
    const totalItems = useAppSelector((state) => state.offersCategories.totalItems);

    const filtered = offersCategoriesList.filter(offersCategories => {
      return word===""?offersCategories:offersCategories.offerCategorieNameInArabic.startsWith(word)||offersCategories.offerCategorieNameInEnglish.startsWith(word);
    });
  ...
  return (
    ...
    <div className="sm:flex sm:items-center ">
      ...
      <div className="sm:flex-auto text-right">
        <h1 className="text-xl font-semibold text-gray-900">فئات العروض</h1>
...
اسم الفئة بالعربي                    
  <span className="ml-2 flex-none rounded bg-gray-200 text-gray-900 group-hover:bg-gray-300">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                ...
                {filtered.map(offersCategories => {
 const provider=providers.find(
  (it) => it.id.toString() === offersCategories.offerProviderId.toString()
)
                  const creationDate = new Date(offersCategories.creationDate);
                  ...
                  return (
                    <tr key={`entity-${offersCategories.id}`} >
                      <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                      <div className="h-10 w-10 flex-shrink-0">
                            <img className="h-10 w-10 rounded-full" src={offersCategories.offerCategorieIconUrl} alt="" />
                          </div>


                     
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{offersCategories.offerCategorieNameInArabic}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{offersCategories.offerCategorieNameInEnglish}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{provider&&provider.providerNameInArabic}</td> 
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500"> {offersCategories.status?<div className=" text-green-400">نعم</div>:<div className=" text-red-400">لا</div>}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{offersCategories.addedBy}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-[#827349] underline">{creationDateString}</td>
              
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <Link to={`/offers-categories/${offersCategories.id}`} className="text-[#827349] hover:text-[#827349]">
                          عرض<span className="sr-only"></span>
                        </Link>
                        <Link to={`/offers-categories/${offersCategories.id}/edit`} className="text-[#827349] hover:text-[#827349] mr-5 ml-5">
                          تعديل  <span className="sr-only"></span>
                        </Link>
                        <Link to={`/offers-categories/${offersCategories.id}/delete`} className="text-[#827349] hover:text-[#827349] ">
