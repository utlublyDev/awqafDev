...
import { getEntity } from "./offers-categories.reducer";
import { getEntitiesproviders } from "../providers/providers.reducer";

export const OffersCategoriesDetail = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const offersCategoriesEntity = useAppSelector(
    ...
    const provider=providers.find(
      (it) => it.id === offersCategoriesEntity?.offerProviderId
    )
  ...
  const creationDate = new Date(offersCategoriesEntity.creationDate);
  ...
  return (
    ...
    <div className="bg-white  rounded-lg shadow-md overflow-hidden sm:rounded-lg"dir="rtl">
      <div className="px-4 py-5 sm:px-6">
        <h3 className="text-lg leading-6 font-medium text-gray-900"> تفاصيل   فئة العروض #   {offersCategoriesEntity.id} </h3>
      ...
      <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
        <dl className="sm:divide-y sm:divide-gray-200">
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <div className="h-16 w-16 flex-shrink-0">
                            <img className="h-16 w-16 rounded-sm" src={offersCategoriesEntity.offerCategorieIconUrl} alt="" />
                          </div>
            </dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">  اسم الفئة بالعربي </dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"> {offersCategoriesEntity.offerCategorieNameInArabic}
              </dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">اسم الفئة بالانجليزي</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{offersCategoriesEntity.offerCategorieNameInEnglish}</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{offersCategoriesEntity.status?<div className=" text-green-400">فعال</div>:<div className=" text-red-400">غير فعال</div>}</td>
</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">اضيف بواسطة</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{offersCategoriesEntity.addedBy}</dd>
          </div>
        ...
        <Button
              ...
              رجوع
                  ...
                  <Button
                     tag={Link}
                     to={`/offers-categories/${offersCategoriesEntity.id}/edit`}
                     replace
...
export default OffersCategoriesDetail;
